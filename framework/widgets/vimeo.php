...
class ZoneVimeoWidget extends WP_Widget 
{
	...
	{
		parent::__construct(
			'vimeo_widget', // Base ID
			__('Zone Vimeo Widget', 'brand'), // Name
			array( 'description' => __( 'A widget that displays a Vimeo player.', THEME_SN ), 'title' => __( 'Vimeo', THEME_SN ) ) // Args
		);
	}
	...
	?>
		...
		<?php if ( $instance["vimeo_description"] != "" ) : ?>				
		<div class="vimeo-desc"><p><?php echo esc_attr( $instance["vimeo_description"] ) ?></p></div>
		<?php endif; ?>
...
function register_vimeo_widget() 
{
    register_widget( 'ZoneVimeoWidget' );
}
add_action( 'widgets_init', 'register_vimeo_widget' );